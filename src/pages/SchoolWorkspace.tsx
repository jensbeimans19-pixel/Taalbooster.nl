...
import { Home, AlertTriangle, Clock, PlayCircle, Flame, Star, User, X, Save, Trash2, LogOut, Check, Trophy, Medal, Brain, Music, Image as ImageIcon, Layout, BarChart3, Gamepad2 } from 'lucide-react';
...
const SchoolWorkspace: React.FC = () => {
  ...
  useEffect(() => {
      const hasAccess = localStorage.getItem('school_access_eenbes') === 'true';
      setIsVerified(hasAccess);
  }, []);

  const handleVerify = (e: React.FormEvent) => {
      e.preventDefault();
      if (accessCode === '5664HB') {
  ...
  const handleSpendStars = (amount: number) => {
      ...
      try {
        const scores = JSON.parse(localStorage.getItem('nt2_scores') || '{}');
        // Deduct from 'nl' bucket primarily, or spread
        // Simplified: Just use 'nl' as main bucket for now
        const current = scores['nl'] || 0;
        if (current >= amount) {
            scores['nl'] = current - amount;
        } else {
            // Edge case: if totalScore is high but 'nl' bucket is low (due to other languages), 
            // this simple logic might fail. But for this app scope, let's assume 'nl' tracks the "Star Bank".
            // A more robust way:
            scores['nl'] = (scores['nl'] || 0) - amount; // Allow negative temporarily or logic ensures check before call
        }
  ...
  const handleLogout = () => {
      // Just removes the name and avatar, keeps the data
  ...
  useEffect(() => {
    loadUserData();
  }, [mode, step, showProfileModal]); // Reload when navigating or closing modal
      ...
      return (
          <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
              <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border-4 border-[#005B8C] max-w-md w-full text-center animate-bounce-in">
                  ...
                  <h1 className="text-2xl font-heading text-[#005B8C] mb-2">Eenbes Basisonderwijs</h1>
                  <p className="text-gray-500 mb-8">Voer de toegangscode in om deze omgeving te openen.</p>
                  ...
                  <form onSubmit={handleVerify} className="space-y-4">
                      <input 
                        ...
                        className="w-full px-6 py-4 rounded-xl border-2 border-gray-200 focus:border-[#8DBF45] focus:outline-none font-mono text-center text-xl uppercase tracking-widest"
                        autoFocus
                      />
                      ...
                      <button 
                        type="submit"
                        className="w-full bg-[#8DBF45] text-white py-4 rounded-xl font-bold hover:bg-[#7ca83d] transition-colors shadow-md text-lg"
                      >
                          Verifiëren
                      </button>
                  </form>
                  ...
                  <p className="mt-6 text-xs text-gray-400">
                      © {new Date().getFullYear()} TaalBooster • Beveiligde Omgeving
                  </p>
              </div>
          </div>
      );
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans selection:bg-[#8DBF45] selection:text-white relative">
                      ...
                      <div className="text-center max-w-md mx-auto">
                            ...
                            {AVATARS.map((avatar, index) => (
                              <button
                                ...
                                className={`relative w-20 h-20 rounded-full overflow-hidden border-4 transition-all transform hover:scale-105 shadow-md ${
                                  tempAvatar === avatar 
                                    ? 'border-[#8DBF45] ring-4 ring-green-100 scale-110' 
                                    : 'border-white ring-2 ring-gray-100 opacity-80 hover:opacity-100'
                                }`}
                              >
                                <img src={avatar} alt={`Avatar ${index + 1}`} className="w-full h-full object-cover" />
                                {tempAvatar === avatar && (
                                  <div className="absolute inset-0 flex items-center justify-center bg-[#8DBF45]/20">
                                    <div className="bg-white rounded-full p-1">
                                      <Check className="w-4 h-4 text-[#8DBF45] stroke-[4]" />
                                    </div>
                                  </div>
                                )}
                              </button>
                          ...
                          <div className="flex gap-2">
                              <input 
                                ...
                                className="flex-1 px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-[#8DBF45] focus:outline-none font-bold text-lg"
                                autoFocus
                              />
                              <button 
                                onClick={handleSaveProfile}
                                disabled={!tempName.trim()}
                                className="bg-[#8DBF45] text-white px-4 rounded-xl hover:bg-[#7ca83d] disabled:opacity-50 transition-colors"
                              >
                      ...
                      <div className="pt-2">
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                               ...
                               <div className="flex flex-col items-center text-center h-full">
                                   <div className="w-24 h-24 rounded-full overflow-hidden mb-3 shadow-md border-4 border-white ring-4 ring-green-100 bg-white">
                               {userAvatar ? (
                                 <img src={userAvatar} alt={userName} className="w-full h-full object-cover" />
                               ) : (
                                 <div className="w-full h-full bg-[#8DBF45] flex items-center justify-center text-white">
                                   ...
                                   <div className="flex flex-col items-center gap-2 mb-6 w-full">
                                       ...
                                       <div className="w-full max-w-[200px] h-3 bg-gray-100 rounded-full overflow-hidden border border-gray-200 relative" title={`${progress}/100 XP tot volgend niveau`}>
                                           <div className="h-full bg-[#8DBF45] transition-all duration-500" style={{ width: `${progress}%` }}></div>
                                       </div>
                                   </div>
        ...
        <div className="max-w-4xl mx-auto px-4 flex items-center justify-between relative">
          ...
          <div className="flex items-center gap-3 sm:gap-4 cursor-pointer z-10" onClick={reset}>
            ...
            <img 
              ...
              alt="TaalBooster" 
              className="h-8 sm:h-10 w-auto object-contain mt-1"
            />
          </div>
          ...
          <div className="z-10 flex items-center justify-end gap-3">
            ...
            <button 
                ...
                className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all border-2 shadow-sm hover:shadow-md active:scale-95 overflow-hidden ${
                    userName 
                    ? 'bg-[#8DBF45] text-white border-white ring-2 ring-green-100 p-0.5' 
